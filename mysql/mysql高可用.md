...
MHA Manager : 可部署在单独的服务器，也可部署在 slave 节点上。会定时监测集群中的 master 节点，当 master 节点出现故障时，会自动的将最新数据的 slave 节点提升为 master。
...
不等主备数据同步，直接把连接切到备库 B，并且让备库 B 可以读写，那么系统几乎就没 有不可用时间了。
